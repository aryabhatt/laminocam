...
namespace tomocam::gpu::opt {
    ...
    namespace detail {
        ...
        // Multiply the R2C output buffer by the ramp filter, broadcasting over
        // the batch (n1 slices). Each thread handles one (s, j, k) element.
        template <typename T>
        __global__ void filter_apply_kernel(cuda::std::complex<T> *work,
                                            const T *filt, size_t n1,
                                            size_t n2, size_t nc) {
            size_t s = blockIdx.z;
            size_t j = blockIdx.y * blockDim.y + threadIdx.y;
            size_t k = blockIdx.x * blockDim.x + threadIdx.x;
            if (s >= n1 || j >= n2 || k >= nc) return;
            work[s * n2 * nc + j * nc + k] *= filt[j * nc + k];
        }
      ...
      public:
        explicit GpuRampPreconditioner(dims_t dims)
            ...
            for (size_t i = 0; i < n2; ++i) {
                yfreq[i] = (i <= n2 / 2) ? (T)i / (T)n2
                                          : (T)((int)i - (int)n2) / (T)n2;
            }
            std::vector<T> h_filt(n2 * nc_);
            for (size_t j = 0; j < n2; ++j) {
                for (size_t k = 0; k < nc_; ++k) {
                    h_filt[j * nc_ + k] = std::sqrt(xfreq[k] * xfreq[k] +
                                                     yfreq[j] * yfreq[j]);
                }
            }
        ...
        GpuRampPreconditioner &operator=(const GpuRampPreconditioner &) =
            delete;
        ...
        DeviceArray<T> apply(const DeviceArray<T> &in) const {
            ...
            auto dims = in.dims();
            // FFT 2D R2C
            auto work = fft::rfft2d(in);
            // Apply filter
            array::bcast_multiply(work, filter_);
            // IFFT 2D C2R
            auto out = fft::irfft2d(work, dims);
            // Normalize by n2*n3 
            T scale = static_cast<T> / static_cast<T>(dims.n2 * dims.n3);
            array::scale_inplace(out, scale);
